...
```
NOTE: Above mangle rule sniffs everything in prerouting but firewall rules don't block anything from bridge, so you might as well specify WAN port in mangle or
      if you want to block devices from bridge, omit in/out-interface=!bridge in firewall.
--------------------------------------------------------------------
Credits for idea:
